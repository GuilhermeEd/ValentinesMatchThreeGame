...
public class ShapesManager : MonoBehaviour
{
  ...
  public readonly Vector2 BottomRight = new Vector2(-2.45f, -4.27f);
  ...
  public IEnumerator InstantiateRandomExplosions(int amount, float interval = 0.25f)
  {
    for (int i = 0; i < amount; i++)
    {
      int row = Random.Range(0, Constants.Rows);
      int col = Random.Range(0, Constants.Columns);
      var item = shapes[row, col];
      GameObject explosion = GetRandomExplosion();
